...
class Expert(Base):
    "Ролевой профиль исполнителя: рейтинг, карта, продажа контактов, уведомления."
    ...
    certificates: Mapped[list[Any] | None] = mapped_column(JSONB, nullable=True)
    ...
    map_fields: Mapped[list[Any] | None] = mapped_column(JSONB, nullable=True)
    ...
    account: Mapped["Account"] = relationship(back_populates="expert_profile", lazy="joined")

    def __str__(self) -> str:
        return f"Исполнитель: {self.account}"

...
export function formatTransactionText(item: PaymentLike): string {
  ...
  const labels: Record<string, { ok: string; cancel: string; pending: string }> = {
    DEPOSIT: {
      ok: `Пополнение баланса +${rub} ₽`,
      cancel: `Пополнение баланса ${rub} ₽ (отменено)`,
      pending: `Пополнение баланса ${rub} ₽ (в обработке)`,
    },
    COMMISSION: {
      ok: `Комиссия за проект −${rub} ₽`,
      cancel: `Комиссия за проект ${rub} ₽ (отменено)`,
      pending: `Комиссия за проект −${rub} ₽ (в обработке)`,
    },
    WITHDRAWAL: {
      ok: `Вывод средств −${rub} ₽`,
      cancel: `Вывод средств ${rub} ₽ (отменено)`,
      pending: `Вывод средств −${rub} ₽ (в обработке)`,
    },
  };

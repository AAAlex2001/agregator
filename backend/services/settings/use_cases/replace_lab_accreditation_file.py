"Use case: заменить файл свидетельства об аккредитации лаборатории."
from fastapi import UploadFile

from models.account import Account
from services.license_holders import remove_regulatory_document_file, save_lab_accreditation_file
from services.settings.repository import SettingsRepository
from services.settings.validators import SettingsValidator


class ReplaceLabAccreditationFileUseCase:
    "Загружает новое свидетельство аккредитации лаборатории и удаляет предыдущее после успешной записи в БД."
    ...
    async def execute(self, user_id: int, file: UploadFile) -> Account:
        ...
        new_url = await save_lab_accreditation_file(owner_key, file)

        previous_url = holder.lab_accreditation_file_url
        holder.lab_accreditation_file_url = new_url
        try:

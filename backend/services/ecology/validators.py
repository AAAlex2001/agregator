...
class EcologyValidator:
    """Проверяет, что анкету открывает исполнитель со своим профилем."""

    def __init__(self, repo: EcologyRepository) -> None:
        self.repo = repo
    ...
    def require_expert(account: Account) -> Expert:
        ...
        if account.role is not UserRole.EXPERT or account.expert_profile is None:
            raise HTTPException(
                ...
                detail="Анкета эколога доступна только исполнителю",
            )

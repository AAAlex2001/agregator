...
class ContactDealRepository:
    ...
    async def get(self, deal_id: int) -> ContactAccessDeal | None:
        query = (
    ...
    async def get_user(self, user_id: int) -> User | None:
        return (
            await self.db.execute(select(User).where(User.id == user_id, User.is_active.is_(True)))
        ).scalars().first()
    ...
    async def find_for_seller_and_buyer(
        self, seller_id: int, buyer_id: int
        ...
        return await self.get(deal_id) if deal_id is not None else None

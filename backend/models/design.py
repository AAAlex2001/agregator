...
class ExpertDesignProfile(Base):
    """Анкета проектировщика: образование, специальности, НОК, НРС и аттестация РТН."""
    __tablename__ = "expert_design_profiles"
    ...
    expert: Mapped["Expert"] = relationship(back_populates="design_profile")


class LicenseHolderDesignProfile(Base):
    """Анкета держателя разрешительных документов — члена СРО проектировщиков.

    Файл выписки из реестра СРО живёт в общей регистрации держателя
    (LicenseHolder.sro_design_file_url); здесь данные членства и условия услуг.
    """
    __tablename__ = "license_holder_design_profiles"

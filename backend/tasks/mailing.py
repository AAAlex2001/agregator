...
async def send_one_batch(
    subject: str, body_text: str, presentation_path: str | None, batch_size: int
) -> None:
    "Разовая отправка одной пачки (background task). Открывает свою сессию БД, без цикла."
    async with AsyncSessionLocal() as db:
        repo = CompanyRepository(db)
        sent = await SendBatchUseCase(repo).execute(subject, body_text, presentation_path, batch_size)
        await db.commit()
